...
bool is_isogram(const char phrase[]) {
    ...
    // make a copy of the const input without spaces or hythens
    char *i = cpy_phrase; // ptr to first element
    for (; *phrase; i++, phrase++) {
        if (*phrase == ' ' || *phrase == '-') {
            phrase++;
            continue;
    ...
    *i = '\0';
    ...
    // search for repeating characters without creating an array of found
